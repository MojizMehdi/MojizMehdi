...
Scenario Outline: As a user i want to Verify Bill Payment From Home Screen Icon
	...
	And I set value in context from database "<IS_SI_Allowed_query>" as "IS_SI_Allowed" on Schema "<db_val>"
	And I set value in context from database "<is_paid_query>" as "IS_PAID_REQ" on Schema "<db_val>"
	And I set value in context from database "<is_partial_payment_query>" as "IS_Partial_Payment" on Schema "<db_val>"
	...
	And verify the result from "<bill_status_id_query>" on Schema "<db_val>"
...
Scenario Outline: 1 As a user i want to Verify Bill Payment through PAY
	...
	And I set value in context from database "<is_partial_payment_query>" as "IS_Partial_Payment" on Schema "<db_val>"
	And I select "<account_no>" on "Pay_BillPayment_accountno"
	And verify through database on "<consumer_label_query>" on Schema "<db_val>" on "Pay_ConsumerLabel_Check"
	And verify the result from "<instrument_type>" on Schema "<db_val2>"
	And I have given "<Pay_BillPayment_ConsumerNo_Value>" on "Pay_BillPayment_ConsumerNo"
	And I am performing on "Pay_BillPayment_NextBtn"
	And I have given "<expiry_date>" on "Pay_Card_Expiry_Date"
...
Scenario Outline: As a user i want to Verify MULTI Bill Payment through PAY
	...
	Examples: 
	|Case|Consumer_No_List|tran_type_query|tran_amount_query|from_account_query|company_name_query|consumer_no_query|db_val|db_val2|account_no|account_type|expiry_date|tran_password|
